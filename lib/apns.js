...
/**
 * @const
 * @desc Reset our signing token every 59 minutes as recommended by Apple
 */
...
class APNS extends EventEmitter {
  ...
  constructor({ team, keyId, signingKey, defaultTopic=null, host=HOST, port=PORT, concurrency=CONCURRENCY, connections=MAX_CONNECTIONS }) {
    ...
    this.on(apnsErrors.expiredProviderToken, () => {
      console.log('received apnsErrors.expiredProviderToken')
   ...
   */
  _getSigningToken() {
    console.log('_getSigningToken()', this._token)
    if (this._token) {
      return this._token
    }
   ...
   */
  _resetSigningToken() {
    console.log('_resetSigningToken')
    this._token = null
